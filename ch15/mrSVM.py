...
class MRsvm(MRJob):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.data = pickle.load(open('D:\notebook\Machine Learning\Machine learning in action\myself_Second\ch15\svmDat27')) #二进制数据无法解码
        self.w = 0
        self.eta = 0.69
        self.dataList = []
    ...
    def reduce(self, _, packedVals): #更新w
        ...
        X = self.data[:, 0:-1]
        wMat = mat(self.w);
        wDelta = mat(zeros(len(self.w))) # Δw应该是偏差
        for index in self.dataList:
            wDelta += float(labels[index]) * X[index, :]  # wDelta += label*dataSet
            # 计算Δw
        eta = 1.0 / (2.0 * self.t)  # calc new: eta # 调整学习率
        # calc new: w = (1.0 - 1/t)*w + (eta/k)*wDelta
        wMat = (1.0 - 1.0 / self.t) * wMat + (eta / self.k) * wDelta # 计算新的w
